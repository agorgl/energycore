...
void gbuffer_init(struct gbuffer* gb, int width, int height)
{
    ...
    for (size_t i = 0; i < array_length(data_texs); ++i) {
        ...
        glBindTexture(GL_TEXTURE_2D, *(data_texs[i].id));
        glTexImage2D(GL_TEXTURE_2D, 0,
                     data_texs[i].ifmt,
                     width, height, 0,
                     data_texs[i].fmt,
                     data_texs[i].pix_dtype,
                     0
        );
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glFramebufferTexture2D(GL_FRAMEBUFFER, data_texs[i].attachment, GL_TEXTURE_2D, *(data_texs[i].id), 0);
    }

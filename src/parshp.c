...
static void parshp__sphere(float const* uv, float* xyz, void*);
static void parshp__hemisphere(float const* uv, float* xyz, void*);
static void parshp__plane(float const* uv, float* xyz, void*);
static void parshp__klein(float const* uv, float* xyz, void*);
static void parshp__cylinder(float const* uv, float* xyz, void*);
static void parshp__torus(float const* uv, float* xyz, void*);
static void parshp__trefoil(float const* uv, float* xyz, void*);
...
static void parshp__copy3(float* result, float const* a)
{
    result[0] = a[0];
    result[1] = a[1];
    result[2] = a[2];
...
static float parshp__dot3(float const* a, float const* b)
{
    return b[0] * a[0]
         + b[1] * a[1]
         + b[2] * a[2];
...
static void parshp__transform3(float* p, float const* x, float const* y, float const* z)
{
    float px = parshp__dot3(p, x);
    float py = parshp__dot3(p, y);
    float pz = parshp__dot3(p, z);
    p[0] = px;
    p[1] = py;
    p[2] = pz;
}

static void parshp__cross3(float* result, float const* a, float const* b)
{
...
static void parshp__mix3(float* d, float const* a, float const* b, float t)
{
...
static void parshp__scale3(float* result, float a)
{
    result[0] *= a;
    result[1] *= a;
    result[2] *= a;
...
static void parshp__normalize3(float* v)
{
    float lsqr = sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
    if (lsqr > 0)
        parshp__scale3(v, 1.0f / lsqr);
}

static void parshp__subtract3(float* result, float const* a)
{
    result[0] -= a[0];
    result[1] -= a[1];
    result[2] -= a[2];
...
static void parshp__add3(float* result, float const* a)
{
    result[0] += a[0];
    result[1] += a[1];
    result[2] += a[2];
...
static float parshp__sqrdist3(float const* a, float const* b)
{
    float dx = a[0] - b[0];
    float dy = a[1] - b[1];
    float dz = a[2] - b[2];
...
static void parshp__compute_welded_normals(struct parshp_mesh* m)
{
    m->normals = PAR_MALLOC(float, m->npoints * 3);
    parshp_index_type_t* weldmap = PAR_MALLOC(parshp_index_type_t, m->npoints);
    struct parshp_mesh* welded = parshp_weld(m, 0.01, weldmap);
    parshp_compute_normals(welded);
...
struct parshp_mesh* parshp_create_cylinder(int slices, int stacks)
{
    if (slices < 3 || stacks < 1)
        return 0;
    return parshp_create_parametric(parshp__cylinder, slices, stacks, 0);
}
...
struct parshp_mesh* parshp_create_parametric_sphere(int slices, int stacks)
{
    if (slices < 3 || stacks < 3)
        return 0;
    struct parshp_mesh* m = parshp_create_parametric(parshp__sphere, slices, stacks, 0);
    parshp_remove_degenerate(m, 0.0001);
    return m;
}
...
struct parshp_mesh* parshp_create_hemisphere(int slices, int stacks)
{
    if (slices < 3 || stacks < 3)
        return 0;
    struct parshp_mesh* m = parshp_create_parametric(parshp__hemisphere, slices, stacks, 0);
    parshp_remove_degenerate(m, 0.0001);
    return m;
}
...
struct parshp_mesh* parshp_create_torus(int slices, int stacks, float radius)
{
    ...
    return parshp_create_parametric(parshp__torus, slices, stacks, userdata);
}
...
struct parshp_mesh* parshp_create_klein_bottle(int slices, int stacks)
{
    if (slices < 3 || stacks < 3)
        return 0;
    struct parshp_mesh* mesh = parshp_create_parametric(parshp__klein, slices, stacks, 0);
    int face = 0;
    for (int stack = 0; stack < stacks; stack++) {
        for (int slice = 0; slice < slices; slice++, face += 2) {
            if (stack < 27 * stacks / 32) {
                parshp_invert(mesh, face, 2);
            }
        }
    }
    parshp__compute_welded_normals(mesh);
    return mesh;
}
...
struct parshp_mesh* parshp_create_trefoil_knot(int slices, int stacks, float radius)
{
    ...
    void* userdata = (void*)&radius;
    return parshp_create_parametric(parshp__trefoil, slices, stacks, userdata);
}
...
struct parshp_mesh* parshp_create_plane(int slices, int stacks)
{
    if (slices < 1 || stacks < 1)
        return 0;
    return parshp_create_parametric(parshp__plane, slices, stacks, 0);
}

struct parshp_mesh* parshp_create_parametric(parshp_fn fn, int slices, int stacks, void* userdata)
{
    struct parshp_mesh* mesh = PAR_CALLOC(struct parshp_mesh, 1);
    ...
    mesh->triangles = PAR_CALLOC(parshp_index_type_t, 3 * mesh->ntriangles);
    int v = 0;
    parshp_index_type_t* face = mesh->triangles;
    for (int stack = 0; stack < stacks; stack++) {
...
void parshp_free_mesh(struct parshp_mesh* mesh)
{
...
void parshp_export(struct parshp_mesh const* mesh, char const* filename)
{
    FILE* objfile = fopen(filename, "wt");
    float const* points = mesh->points;
    float const* tcoords = mesh->tcoords;
    float const* norms = mesh->normals;
    parshp_index_type_t const* indices = mesh->triangles;
    if (tcoords && norms) {
...
static void parshp__sphere(float const* uv, float* xyz, void* userdata)
{
    (void)userdata;
...
static void parshp__hemisphere(float const* uv, float* xyz, void* userdata)
{
    (void)userdata;
    float phi = uv[0] * PAR_PI;
...
static void parshp__plane(float const* uv, float* xyz, void* userdata)
{
    (void)userdata;
    xyz[0] = uv[0];
    xyz[1] = uv[1];
...
static void parshp__klein(float const* uv, float* xyz, void* userdata)
{
    (void)userdata;
    float u = uv[0] * PAR_PI;
...
static void parshp__cylinder(float const* uv, float* xyz, void* userdata)
{
    (void)userdata;
    float theta = uv[1] * 2 * PAR_PI;
    xyz[0] = sinf(theta);
...
static void parshp__torus(float const* uv, float* xyz, void* userdata)
{
    float major = 1;
...
static void parshp__trefoil(float const* uv, float* xyz, void* userdata)
{
    float minor = *((float*)userdata);
    const float a = 0.5f;
    const float b = 0.3f;
    ...
    parshp__normalize3(q);
    float qvn[3] = {q[1], -q[0], 0};
    parshp__normalize3(qvn);
    float ww[3];
    parshp__cross3(ww, q, qvn);
...
{
    parshp_index_type_t offset = dst->npoints;
    ...
    dst->triangles = PAR_REALLOC(parshp_index_type_t, dst->triangles, 3 * ntriangles);
    parshp_index_type_t* ptriangles = dst->triangles + 3 * dst->ntriangles;
    parshp_index_type_t const* striangles = src->triangles;
...
struct parshp_mesh* parshp_create_disk(float radius, int slices, float const* center, float const* normal)
{
    ...
    *points++ = 0;
    ...
    mesh->triangles = PAR_MALLOC(parshp_index_type_t, 3 * mesh->ntriangles);
    parshp_index_type_t* triangles = mesh->triangles;
    for (int i = 0; i < slices; i++) {
        *triangles++ = 0;
        *triangles++ = 1 + i;
    ...
    float k[3] = {0, 0, -1};
    float axis[3];
    parshp__cross3(axis, nnormal, k);
    parshp__normalize3(axis);
    parshp_rotate(mesh, acos(nnormal[2]), axis);
    parshp_translate(mesh, center[0], center[1], center[2]);
    return mesh;
}

struct parshp_mesh* parshp_create_empty() { return PAR_CALLOC(struct parshp_mesh, 1); }

void parshp_translate(struct parshp_mesh* m, float x, float y, float z)
{
...
void parshp_rotate(struct parshp_mesh* mesh, float radians, float const* axis)
{
    ...
    float col0[3] = {(((x * x) * one_minus_c) + c), ((xy * one_minus_c) + (z * s)), ((zx * one_minus_c) - (y * s))};
    float col1[3] = {((xy * one_minus_c) - (z * s)), (((y * y) * one_minus_c) + c), ((yz * one_minus_c) + (x * s))};
    float col2[3] = {((zx * one_minus_c) + (y * s)), ((yz * one_minus_c) - (x * s)), (((z * z) * one_minus_c) + c)};
    float* p = mesh->points;
...
void parshp_scale(struct parshp_mesh* m, float x, float y, float z)
{
    float* points = m->points;
    for (int i = 0; i < m->npoints; i++) {
        *points++ *= x;
...
{
    parshp_merge(dst, src);
    parshp_free_mesh(src);
}

void parshp_compute_aabb(struct parshp_mesh const* m, float* aabb)
{
    float* points = m->points;
...
void parshp_invert(struct parshp_mesh* m, int face, int nfaces)
{
    nfaces = nfaces ? nfaces : m->ntriangles;
    parshp_index_type_t* tri = m->triangles + face * 3;
    for (int i = 0; i < nfaces; i++) {
        PAR_SWAP(parshp_index_type_t, tri[0], tri[2]);
        tri += 3;
    }
}
...
struct parshp_mesh* parshp_create_icosahedron()
{
    ...
    static parshp_index_type_t faces[] = { 0, 1,  2,  0,  2, 3,  0, 3,  4,  0, 4, 5, 0, 5, 1,  7, 6, 11, 8, 7,
    ...
    mesh->triangles = PAR_MALLOC(parshp_index_type_t, sizeof(faces) / 2);
...
struct parshp_mesh* parshp_create_dodecahedron()
{
    ...
    static parshp_index_type_t pentagons[12 * 5] = { 0,  1,  2, 3,  4,  5, 10,  6, 1,  0,  6, 11,  7,  2,  1,
    ...
    memcpy(mesh->points, verts, sizeof(verts));
    parshp_index_type_t const* pentagon = pentagons;
    mesh->ntriangles = npentagons * 3;
    mesh->triangles = PAR_MALLOC(parshp_index_type_t, mesh->ntriangles * 3);
    parshp_index_type_t* tris = mesh->triangles;
...
struct parshp_mesh* parshp_create_octahedron()
{
    ...
    static parshp_index_type_t triangles[8 * 3] = {
    ...
    mesh->points = PAR_MALLOC(float, mesh->npoints * 3);
    memcpy(mesh->points, verts, sizeof(verts));
    parshp_index_type_t const* triangle = triangles;
...
struct parshp_mesh* parshp_create_tetrahedron()
{
    ...
    static parshp_index_type_t triangles[4 * 3] = {
...
struct parshp_mesh* parshp_create_cube()
{
    ...
    static parshp_index_type_t quads[6 * 4] = {
        7, 6, 5, 4, /* front  */
        0, 1, 2, 3, /* back   */
        6, 7, 3, 2, /* right  */
        5, 6, 2, 1, /* top    */
    ...
    mesh->points = PAR_MALLOC(float, mesh->npoints * 3);
    memcpy(mesh->points, verts, sizeof(verts));
    parshp_index_type_t const* quad = quads;
...
typedef struct {
    ...
    float position[3];
    float scale[3];
    struct parshp_mesh* orientation;
    parshp__rule* rule;
} parshp__stackframe;

static parshp__rule* parshp__pick_rule(const char* name, parshp__rule* rules, int nrules)
{
    parshp__rule* rule = 0;
    int total = 0;
...
static struct parshp_mesh* parshp__create_turtle()
{
    const float xaxis[] = {1, 0, 0};
    const float yaxis[] = {0, 1, 0};
    const float zaxis[] = {0, 0, 1};
    struct parshp_mesh* turtle = PAR_CALLOC(struct parshp_mesh, 1);
    turtle->npoints = 3;
    turtle->points = PAR_CALLOC(float, turtle->npoints * 3);
    parshp__copy3(turtle->points + 0, xaxis);
    parshp__copy3(turtle->points + 3, yaxis);
    parshp__copy3(turtle->points + 6, zaxis);
    return turtle;
}

static struct parshp_mesh* parshp__apply_turtle(struct parshp_mesh* mesh, struct parshp_mesh* turtle, float const* pos,
                                                 float const* scale)
{
    struct parshp_mesh* m = parshp_clone(mesh, 0);
    for (int p = 0; p < m->npoints; p++) {
        float* pt = m->points + p * 3;
        pt[0] *= scale[0];
        pt[1] *= scale[1];
        pt[2] *= scale[2];
        parshp__transform3(pt, turtle->points + 0, turtle->points + 3, turtle->points + 6);
        pt[0] += pos[0];
        pt[1] += pos[1];
        pt[2] += pos[2];
    }
...
static void parshp__connect(struct parshp_mesh* scene, struct parshp_mesh* cylinder, int slices)
{
    int stacks = 1;
    ...
    parshp_index_type_t* triangles = PAR_MALLOC(parshp_index_type_t, ntriangles * 3);
    memcpy(triangles, scene->triangles, 2 * scene->ntriangles * 3);
    int v = scene->npoints - (slices + 1);
    parshp_index_type_t* face = triangles + scene->ntriangles * 3;
    for (int stack = 0; stack < stacks; stack++) {
        for (int slice = 0; slice < slices; slice++) {
            int next = slice + 1;
            *face++ = v + slice + slices + 1;
...
struct parshp_mesh* parshp_create_lsystem(char const* text, int slices, int maxdepth)
{
    char* program;
    ...
    /* Allocate space. */
    parshp__rule* rules = PAR_MALLOC(parshp__rule, nrules);
    parshp__command* commands = PAR_MALLOC(parshp__command, ncommands);

    /* Initialize the entry rule. */
    parshp__rule* current_rule = &rules[0];
    parshp__command* current_command = &commands[0];
...
/* For testing purposes, dump out the parsed program. */
#ifdef TEST_PARSE
    for (int i = 0; i < nrules; i++) {
        parshp__rule rule = rules[i];
        printf("rule %s.%d\n", rule.name, rule.weight);
        for (int c = 0; c < rule.ncommands; c++) {
            parshp__command cmd = rule.commands[c];
            printf("\t%s %s\n", cmd.cmd, cmd.arg);
        }
    }
#endif
    ...
    struct parshp_mesh* scene = PAR_CALLOC(struct parshp_mesh, 1);
    struct parshp_mesh* tube = parshp_create_cylinder(slices, 1);
    struct parshp_mesh* turtle = parshp__create_turtle();
    ...
    parshp__stackframe* stack = PAR_CALLOC(parshp__stackframe, maxdepth);
    int stackptr = 0;
    stack[0].orientation = turtle;
    stack[0].rule = &rules[0];
    parshp__copy3(stack[0].scale, units);
    while (stackptr >= 0) {
        parshp__stackframe* frame = &stack[stackptr];
        parshp__rule* rule = frame->rule;
        struct parshp_mesh* turtle = frame->orientation;
        float* position = frame->position;
        float* scale = frame->scale;
        if (frame->pc >= rule->ncommands) {
            parshp_free_mesh(turtle);
            stackptr--;
            continue;
        }

        parshp__command* cmd = rule->commands + (frame->pc++);
#ifdef DUMP_TRACE
        ...
        if (!strcmp(cmd->cmd, "shape")) {
            struct parshp_mesh* m = parshp__apply_turtle(tube, turtle, position, scale);
            if (!strcmp(cmd->arg, "connect")) {
                parshp__connect(scene, m, slices);
            } else {
                parshp_merge(scene, m);
            }
            parshp_free_mesh(m);
        } else if (!strcmp(cmd->cmd, "call") && stackptr < maxdepth - 1) {
            rule = parshp__pick_rule(cmd->arg, rules, nrules);
            frame = &stack[++stackptr];
            frame->rule = rule;
            frame->orientation = parshp_clone(turtle, 0);
            frame->pc = 0;
            parshp__copy3(frame->scale, scale);
            parshp__copy3(frame->position, position);
            continue;
        } else {
            value = atof(cmd->arg);
            if (!strcmp(cmd->cmd, "rx")) {
                parshp_rotate(turtle, value * PAR_PI / 180.0, xaxis);
            } else if (!strcmp(cmd->cmd, "ry")) {
                parshp_rotate(turtle, value * PAR_PI / 180.0, yaxis);
            } else if (!strcmp(cmd->cmd, "rz")) {
                parshp_rotate(turtle, value * PAR_PI / 180.0, zaxis);
            } else if (!strcmp(cmd->cmd, "tx")) {
                float vec[3] = {value, 0, 0};
                float t[3] = {parshp__dot3(turtle->points + 0, vec), parshp__dot3(turtle->points + 3, vec),
                              parshp__dot3(turtle->points + 6, vec)};
                parshp__add3(position, t);
...
void parshp_unweld(struct parshp_mesh* mesh, int create_indices)
{
    ...
    float* dst = points;
    parshp_index_type_t const* index = mesh->triangles;
    ...
    if (create_indices) {
        parshp_index_type_t* tris = PAR_MALLOC(parshp_index_type_t, 3 * mesh->ntriangles);
        parshp_index_type_t* index = tris;
...
void parshp_compute_normals(struct parshp_mesh* m)
{
    ...
    parshp_index_type_t const* triangle = m->triangles;
    ...
    for (int f = 0; f < m->ntriangles; f++, triangle += 3) {
        ...
        parshp__copy3(next, pb);
        parshp__subtract3(next, pa);
        parshp__copy3(prev, pc);
        parshp__subtract3(prev, pa);
        parshp__cross3(cp, next, prev);
        parshp__add3(m->normals + 3 * triangle[0], cp);
        parshp__copy3(next, pc);
        parshp__subtract3(next, pb);
        parshp__copy3(prev, pa);
        parshp__subtract3(prev, pb);
        parshp__cross3(cp, next, prev);
        parshp__add3(m->normals + 3 * triangle[1], cp);
        parshp__copy3(next, pa);
        parshp__subtract3(next, pc);
        parshp__copy3(prev, pb);
        parshp__subtract3(prev, pc);
        parshp__cross3(cp, next, prev);
        parshp__add3(m->normals + 3 * triangle[2], cp);
    }
    float* normal = m->normals;
    for (int p = 0; p < m->npoints; p++, normal += 3) {
        parshp__normalize3(normal);
    }
}

static void parshp__subdivide(struct parshp_mesh* mesh)
{
    ...
    for (int t = 0; t < mesh->ntriangles; t++, spoint += 9, dpoint += 3) {
        ...
        parshp__mix3(dpoint, a, b, 0.5);
        parshp__mix3(dpoint += 3, b, c, 0.5);
        parshp__mix3(dpoint += 3, a, c, 0.5);
        parshp__add3(dpoint += 3, a);
        parshp__add3(dpoint += 3, p0);
        parshp__add3(dpoint += 3, p2);
        parshp__add3(dpoint += 3, p0);
        parshp__add3(dpoint += 3, b);
        parshp__add3(dpoint += 3, p1);
        parshp__add3(dpoint += 3, p2);
        parshp__add3(dpoint += 3, p1);
        parshp__add3(dpoint += 3, c);
    }
    PAR_FREE(mesh->points);
    mesh->points = points;
    mesh->npoints = npoints;
...
struct parshp_mesh* parshp_create_subdivided_sphere(int nsubd)
{
    struct parshp_mesh* mesh = parshp_create_icosahedron();
    parshp_unweld(mesh, 0);
    PAR_FREE(mesh->triangles);
    mesh->triangles = 0;
    while (nsubd--)
        parshp__subdivide(mesh);
    for (int i = 0; i < mesh->npoints; i++)
        parshp__normalize3(mesh->points + i * 3);
    mesh->triangles = PAR_MALLOC(parshp_index_type_t, 3 * mesh->ntriangles);
    for (int i = 0; i < mesh->ntriangles * 3; i++)
        mesh->triangles[i] = i;
    struct parshp_mesh* tmp = mesh;
    mesh = parshp_weld(mesh, 0.01, 0);
    parshp_free_mesh(tmp);
    parshp_compute_normals(mesh);
    return mesh;
}

struct parshp_mesh* parshp_create_rock(int seed, int subd)
{
    struct parshp_mesh* mesh = parshp_create_subdivided_sphere(subd);
...
struct parshp_mesh* parshp_clone(struct parshp_mesh const* mesh, struct parshp_mesh* clone)
{
    if (!clone)
        clone = PAR_CALLOC(struct parshp_mesh, 1);
    ...
    clone->triangles = PAR_REALLOC(parshp_index_type_t, clone->triangles, 3 * clone->ntriangles);
    memcpy(clone->triangles, mesh->triangles, sizeof(parshp_index_type_t) * 3 * clone->ntriangles);
    if (mesh->normals) {
...
static struct {
    float const* points;
    int gridsize;
} parshp__sort_context;

static int parshp__cmp1(const void* arg0, const void* arg1)
{
    const int g = parshp__sort_context.gridsize;

    /* Convert arg0 into a flattened grid index. */
    parshp_index_type_t d0 = *(const parshp_index_type_t*)arg0;
    float const* p0 = parshp__sort_context.points + d0 * 3;
    int i0 = (int)p0[0];
    int j0 = (int)p0[1];
    int k0 = (int)p0[2];
    ...
    parshp_index_type_t d1 = *(const parshp_index_type_t*)arg1;
    float const* p1 = parshp__sort_context.points + d1 * 3;
    int i1 = (int)p1[0];
    int j1 = (int)p1[1];
    int k1 = (int)p1[2];
...
static void parshp__sort_points(struct parshp_mesh* mesh, int gridsize, parshp_index_type_t* sortmap)
{
    ...
    for (int i = 0; i < mesh->npoints; i++)
        sortmap[i] = i;
    parshp__sort_context.gridsize = gridsize;
    parshp__sort_context.points = mesh->points;
    qsort(sortmap, mesh->npoints, sizeof(parshp_index_type_t), parshp__cmp1);

    /* Apply the reorder mapping to the XYZ coordinate data. */
    float* newpts = PAR_MALLOC(float, mesh->npoints * 3);
    parshp_index_type_t* invmap = PAR_MALLOC(parshp_index_type_t, mesh->npoints);
    float* dstpt = newpts;
    for (int i = 0; i < mesh->npoints; i++) {
        invmap[sortmap[i]] = i;
    ...
    parshp_index_type_t* newinds = PAR_MALLOC(parshp_index_type_t, mesh->ntriangles * 3);
    parshp_index_type_t* dstind = newinds;
    parshp_index_type_t const* srcind = mesh->triangles;
    for (int i = 0; i < mesh->ntriangles * 3; i++)
    ...
    /* Cleanup. */
    memcpy(sortmap, invmap, sizeof(parshp_index_type_t) * mesh->npoints);
    PAR_FREE(invmap);
}

static void parshp__weld_points(struct parshp_mesh* mesh, int gridsize, float epsilon, parshp_index_type_t* weldmap)
{
    ...
    parshp_index_type_t* bins = PAR_CALLOC(parshp_index_type_t, gridsize * gridsize * gridsize);
    int prev_binindex = -1;
    for (int p = 0; p < mesh->npoints; p++) {
    ...
    for (int p = 0; p < mesh->npoints; p++, pt += 3) {
        ...
        for (int i = minp[0]; i <= maxp[0]; i++) {
            for (int j = minp[1]; j <= maxp[1]; j++) {
                for (int k = minp[2]; k <= maxp[2]; k++) {
                    ...
                    parshp_index_type_t binvalue = *(bins + binindex);
                    if (binvalue > 0) {
                        if (nbins == 8) {
        ...
        for (int b = 0; b < nbins; b++) {
            ...
            parshp_index_type_t binvalue = *(bins + binindex);
            parshp_index_type_t nindex = binvalue - 1;
            while (1) {
                ...
                if (nindex != p && weldmap[nindex] == nindex) {
                    ...
                    float dist2 = parshp__sqrdist3(thatpt, pt);
                    if (dist2 < epsilon) {
                        weldmap[nindex] = p;
                        nremoved++;
                    }
    ...
    float* dst = newpts;
    parshp_index_type_t* condensed_map = PAR_MALLOC(parshp_index_type_t, mesh->npoints);
    parshp_index_type_t* cmap = condensed_map;
    ...
    assert(ci == npoints);
    PAR_FREE(mesh->points);
    memcpy(weldmap, condensed_map, mesh->npoints * sizeof(parshp_index_type_t));
    PAR_FREE(condensed_map);
    mesh->points = newpts;
    mesh->npoints = npoints;

    /* Apply the weldmap to the triangle indices and skip the degenerates. */
    parshp_index_type_t const* tsrc = mesh->triangles;
    parshp_index_type_t* tdst = mesh->triangles;
    int ntriangles = 0;
    for (int i = 0; i < mesh->ntriangles; i++, tsrc += 3) {
        parshp_index_type_t a = weldmap[tsrc[0]];
        parshp_index_type_t b = weldmap[tsrc[1]];
        parshp_index_type_t c = weldmap[tsrc[2]];
        if (a != b && a != c && b != c) {
            *tdst++ = a;
            *tdst++ = b;
            *tdst++ = c;
...
struct parshp_mesh* parshp_weld(struct parshp_mesh const* mesh, float epsilon, parshp_index_type_t* weldmap)
{
    struct parshp_mesh* clone = parshp_clone(mesh, 0);
    float aabb[6];
    int gridsize = 20;
    float maxcell = gridsize - 1;
    parshp_compute_aabb(clone, aabb);
    float scale[3] = {
        aabb[3] == aabb[0] ? 1.0f : maxcell / (aabb[3] - aabb[0]),
        aabb[4] == aabb[1] ? 1.0f : maxcell / (aabb[4] - aabb[1]),
        aabb[5] == aabb[2] ? 1.0f : maxcell / (aabb[5] - aabb[2]),
    };
    parshp_translate(clone, -aabb[0], -aabb[1], -aabb[2]);
    parshp_scale(clone, scale[0], scale[1], scale[2]);
    parshp_index_type_t* sortmap = PAR_MALLOC(parshp_index_type_t, mesh->npoints);
    parshp__sort_points(clone, gridsize, sortmap);
    int owner = 0;
    if (!weldmap) {
        owner = 1;
        weldmap = PAR_MALLOC(parshp_index_type_t, mesh->npoints);
    }
    for (int i = 0; i < mesh->npoints; i++)
        weldmap[i] = i;
    parshp__weld_points(clone, gridsize, epsilon, weldmap);
    if (owner) {
        PAR_FREE(weldmap);
    } else {
        parshp_index_type_t* newmap = PAR_MALLOC(parshp_index_type_t, mesh->npoints);
        for (int i = 0; i < mesh->npoints; i++) {
            newmap[i] = weldmap[sortmap[i]];
        }
        memcpy(weldmap, newmap, sizeof(parshp_index_type_t) * mesh->npoints);
        PAR_FREE(newmap);
    }
    PAR_FREE(sortmap);
    parshp_scale(clone, 1.0 / scale[0], 1.0 / scale[1], 1.0 / scale[2]);
    parshp_translate(clone, aabb[0], aabb[1], aabb[2]);
    return clone;
}
...
void parshp_remove_degenerate(struct parshp_mesh* mesh, float mintriarea)
{
    int ntriangles = 0;
    parshp_index_type_t* triangles = PAR_MALLOC(parshp_index_type_t, mesh->ntriangles * 3);
    parshp_index_type_t* dst = triangles;
    parshp_index_type_t const* src = mesh->triangles;
    float next[3], prev[3], cp[3];
    ...
    for (int f = 0; f < mesh->ntriangles; f++, src += 3) {
        ...
        parshp__subtract3(prev, pa);
        parshp__cross3(cp, next, prev);
        float cplen2 = parshp__dot3(cp, cp);
        if (cplen2 >= mincplen2) {
            *dst++ = src[0];
            *dst++ = src[1];
            *dst++ = src[2];
